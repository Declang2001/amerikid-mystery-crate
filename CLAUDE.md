...
- [ ] Create `public/hats/` folder
- [ ] Add 5 hat PNG images (512x512 recommended, transparent background)
- [ ] Update `hats` array (lines 126-132) to use `/hats/*.png` paths instead of `makeHatSvg()`
- [ ] Delete or comment out `makeHatSvg()` function (lines 149-166) once real images work

### Priority 2: Audio Implementation
- [ ] Add `public/audio/mystery-jingle.mp3` (or .ogg)
- [ ] Create `mysteryAudio = new Audio(...)` after line 54 (with other DOM elements)
- [ ] Add `mysteryAudio.play()` at line 330 (start of spin)
- [ ] Add fade-out logic at line 342 (winner selected)
- [ ] Test on iOS Safari (autoplay restrictions are strictest there)

### Priority 3: Carousel Enhancement (Optional)
- [ ] Decide: Simple flash (current) vs. scrolling reel (new)
- [ ] If scrolling reel:
  - Create `.result-carousel` container with 5 `<img>` elements
  - Add CSS `transform: translateX()` animation
  - Replace `setInterval` logic with CSS animation + `animationend` event
  - Add easing curve (start slow, speed up, slow down at end)

### Priority 4: Rarity System (Optional)
- [ ] Add `rarity` field to each hat object (`common`, `rare`, `legendary`)
- [ ] Implement weighted random selection (replace `Math.random() * hats.length`)
- [ ] Add visual rarity indicators (border color, glow effect, etc.)
- [ ] Display rarity in UI (e.g., "RARE" badge)

### Priority 5: Polish
- [ ] Add winner "reveal" animation (scale up, glow, confetti)
- [ ] Add claim confirmation (prevent accidental clicks)
- [ ] Add reset button to spin again after claiming
- [ ] Consider adding a "history" of last 3-5 wins

### Files to Touch
- `public/hats/*` - Hat image assets (PNG with transparency)
- `public/audio/*` - Mystery jingle audio file (MP3/OGG)
- `src/main.js` - Update hats array, add audio integration, optional carousel
- `src/style.css` - Optional: carousel styles, reveal animations

### Current State
- Background: `room.png` loaded via sky-dome sphere (inside-out geometry) with PMREM environment map for reflections
- Hats: Currently placeholder SVGs generated by `makeHatSvg()` with gradient fills
- Spin mechanic: Interval-based single-image swap (160ms cycles), NOT a scrolling carousel
- Winner selection: Pure random (uniform distribution, no rarity weighting)
- State machine: READY → OPENING → SPINNING → WINNER_SELECTED → CLAIMING → CLAIMED
