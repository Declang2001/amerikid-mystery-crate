...
> Status: Iframe UI polish pass applied, then corrective pass on 2026-04-15: (a) Press X keycap reverted from tactical amber beveled clip-path back to the original blue controller-circle look (`.press-x-key` = 24x24 `#4a90e2` circle, `border-radius: 50%`, white text, `line-height: 24px`, hover brightens to `#5ea0ee` with `rgba(74,144,226,0.55)` glow, active drops to `#3d7fcf` + 1px translate); outer `.press-x-prompt` Black Ops One text styling kept because it is the authored feel the user wanted preserved; DOM `<span class="press-x-label">Press</span><span class="press-x-key">X</span><span class="press-x-label">For a Random Hat</span>` kept; event handlers / positioning math / `display: block`/`none` runtime toggle untouched. (b) Purple side stripe removed entirely: `--brand-purple` variable deleted from the tactical `:root` block and the `inset 2px 0 0 var(--brand-purple),` prepend dropped from the 6 additive polish-pass rules on `.boot-copy`, `.boot-idle-copy`, `.panel`, `.result-card`, `.saved-hat-card`, `.size-select-card` (the entire additive block was removed, so those elements fall back cleanly to their pre-polish base `box-shadow` rules declared earlier in the file). (c) 2D UI chrome retoned from gold/amber to official site purple `#ff33ff`: `--hud-amber: #ff33ff`, `--hud-amber-soft: rgba(255,51,255,0.62)`, `--hud-gold: #ff66ff`, `--hud-scanline: rgba(255,51,255,0.042)`; all amber RGBA literals across popup cards, result card chrome, panel accents, button chrome, corner/frame brackets, scanlines, telemetry chip borders, media L-ticks, button depress rings, and HUD edge accents were swapped to their magenta counterparts (main amber `rgba(255,209,106,X) / rgba(255,200,85,X) / rgba(255,210,110,X) / rgba(255,214,118,X)` to `rgba(255,51,255,X)`; light highlights `rgba(255,232,172,X) / rgba(255,232,170,X) / rgba(255,231,184,X) / rgba(255,226,160,X) / rgba(255,222,152,X) / rgba(255,220,160,X) / rgba(255,220,150,X)` to `rgba(255,102,255,X)`; paler highlights `rgba(255,240,190,X) / rgba(255,233,190,X)` to `rgba(255,153,255,X)`; very pale `rgba(255,249,229,X) / rgba(255,245,216,X)` to `rgba(255,220,255,X)`; dark amber CTA backgrounds `rgba(60,44,14,X) / rgba(28,20,8,X)` to dark plum `rgba(56,16,72,X) / rgba(28,8,36,X)`; hex `#ffd16a` to `#ff33ff`, `#f5a524` to `#b520b5`, `#ffc855` to `#ff66ff`, `#ffe4a0` and `#ffe7a4` to `#ff99ff`, `#fff5d6` to `#ffe6ff`, `#ffeab8` to `#ffccff`). Preview cyan (`rgba(140,230,255,X)`, `var(--hud-cyan)`, `#8ce6ff`) was NOT touched because it is a route-specific load-bearing semantic. `.eligibility-status` / `.status-line` / `.result-media::after` / button press-down depress behavior all retained, just retoned to magenta. Winner-reveal 42% keyframe continues to hold its distinct state via the existing `hud-frame-pulse` 2.4s 0.78-to-1.0 opacity animation now running against magenta instead of gold -- no separate hue needed. No DOM restructure. No JS behavior change. No state-machine/audio/camera/scene/API/theme change. Click SFX wiring from the prior pass (boot/saved-hat/size-select/spin buttons; Press X / canvas / keyboard X / `#claimBtn` deliberately excluded) retained. Compact size-select popup retained. Combo product image in size-select retained. No DOM restructure. No JS behavior change. No state-machine/audio/camera/scene/API/theme change. Earlier: preview path redirects directly to a Shopify cart permalink (no intermediate combo product page). Two-popup flow: (1) `showPreviewConfirmOverlay` (copy `PREVIEW SELECTED` / `THIS IS THE HAT YOU'RE TAKING INTO CHECKOUT.` / `CONTINUE`) reuses the `#savedHatOverlay` DOM shell; Continue chains to (2) a new `#sizeSelectOverlay` (`showPreviewSizeOverlay`) with shirt-size options S/M/L/XL/2XL and a `GO TO CHECKOUT` CTA that enables once a size is selected. CTA redirects to `https://amerikid.ca/cart/<comboVariantId>:1?properties=<URL-escaped base64 of {"_preview_hat_id":"<HAT-ID>"}>` (base64 via `btoa`, then `encodeURIComponent`). URL-encoded-JSON payload was tried first and produced a Shopify error page on this store; base64 matches the manually verified working pattern. Combo variant IDs mapped per size in `COMBO_VARIANT_BY_SIZE` (S=51878170034456, M=51878170067224, L=51878170099992, XL=51878170132760, 2XL=51878170165528). New `buildPreviewCartPermalink(hatId, size)` helper constructs the URL. Preview overlay still never sets `savedHatOverlayShown`, never touches `crate.savedHat.pending`; `#savedHatContinueBtn` branches on a stashed `previewContinueAction`. Older `buildComboCheckoutUrl(hat?)` and `buildPreviewCheckoutUrl` helpers retained in code but dormant. Preview finalize handler plays claim SFX, sets CLAIMING, closes crate, opens overlay chain. Purchased path, pending-result bridge, saved-hat immediate-show, purchased email-link route, `/api/*`, inventory-aware landing, state machine, audio, camera, scene, and `buildPreviewCheckoutUrl` (still dormant) are all unchanged. Theme pickup of `preview_hat_id` -> `_preview_hat_id` line-item property and the draft-safe Flow remain downstream work not altered by this pass. Saved-hat confirmation popup moved to immediate post-finalize (shown from inside the purchased-path `closeCrate().then` block right after `setState(STATES.CLAIMED)`, reload now gated on Continue click; `POST_CLAIM_RELOAD_HOLD_MS` auto-reload timer removed; post-reload failsafe trigger + sessionStorage `crate.savedHat.pending` flag retained for force-reload recovery). One-time post-save confirmation card added (sessionStorage key `crate.savedHat.pending` written inside the post-claim reset before reload; on next load, after eligibility resolves and boot reaches `CRATE_VIEW`, a single-use overlay shows the saved hat image, name, and "Your hat is locked in for fulfillment" copy, cross-checked against `eligibility.hatWon` and cleared on display; no server or theme change). Pending-result bridge added (fallback-only tag `crate_pending_result:<HAT-ID>:<UNIX-MS>`, written on winner-landed, cleared on finalize, resumed on reload when hat_won is null), preview CTA redirected to the $50 combo product page (exact-hat preview checkout retained in code for future reuse), entitlement model implemented, purchased flow corrected, 15-hat pool wired, inventory-aware pool filtering implemented, audio mix balanced (spin 0.30, SFX 0.25/1.0, ambient 0.10/0.08/0.05-spin), spin-start variation added, spin easing wind-up added, winner-only prestige pass added (gold grail glow, impact/sustain/settle envelope, steady hold), front-half spin cadence calmed (EXTRA_FULL_ROTATIONS_MAX 2 to 1), tactical HUD polish applied to boot cards and result panel (classified-console corner brackets, scanlines, Black Ops One titles, beveled tactical buttons, cyan/amber/gold state language), post-claim reset added (purchased-path CLAIMED holds briefly then reloads to intro), intro MP4 reliability pass applied (idle.mp4 preload link in index.html, walk download serialized behind idle loadeddata, post-parse crossOrigin mutation removed, idle/walk timeout watchdog + Tap To Retry fallback), hat texture preload deferred off the first-paint critical path (placeholder Textures at module init, deferred kickHatTexturePreload with fetchPriority="low" wired into idle readiness so the ~63 MB hat pool no longer competes with idle.mp4 on mobile cold boot), GPU texture pre-upload added (`renderer.initTexture` called once per hat on image load so the spin reel no longer triggers lazy GPU uploads per swap), UI button click SFX added (`clickSfx` = `/sfx/click.mp3`, volume 0.12, `preload='auto'`, registered in `allAudioElements` for gesture unlock, wired into `#bootStartBtn` / `#bootEnterPortalBtn` / `#savedHatContinueBtn` / `.size-select-option` / `#sizeSelectCtaBtn` / `#spinBtn`; Press X prompt / canvas click / keyboard X / `#claimBtn` deliberately excluded -- `#claimBtn` already plays `claimSfx`). Combo product image added to `#sizeSelectOverlay` card (static `<img>` from Shopify CDN, lazy-loaded, placed between subcopy and size options host). Size-select popup compacted: `.size-select-card` width `min(340px, 92vw)`, tighter padding/gap; `.size-select-product-img` max 120px desktop / 96px <=820px; `.size-select-options` switched to CSS grid (`repeat(5, minmax(0, 1fr))` desktop, `repeat(3, ...)` <=820px) so size buttons sit on a single row on desktop and at most two rows on mobile; smaller option padding and CTA padding to keep popup inside short iframe viewports without scroll.
...
- Launch pool: **24 hats** (implemented in `src/hats.js` and `api/_lib/allowed-hats.js`)
