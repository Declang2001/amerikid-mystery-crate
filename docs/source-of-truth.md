...
> Status: Entitlement model implemented, purchased flow corrected, 15-hat pool wired, exact preview checkout wired, inventory-aware pool filtering implemented.
...
| Spin input (Press X / prompt click / canvas tap) fails in Shopify iframe | Fixed: pointerdown listeners + prompt converted from div to button + user-select/touch-action suppression on prompt and canvas |
