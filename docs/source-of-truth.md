...
- Clicking "Buy Combo Pack" now opens a preview-only confirmation overlay (reuses the `#savedHatOverlay` DOM with distinct preview copy that never implies saved/finalized/locked). The overlay's "CONTINUE TO COMBO PACK" button redirects the user to the $50 combo product page with the landed hat id encoded as a query param: `https://amerikid.ca/products/candyfacts-mystery-box-combo?preview_hat_id=<HAT-ID>`. The revealed hat remains illustrative until the theme-side pickup converts `preview_hat_id` into a `_preview_hat_id` line-item property at add-to-cart.
